// Shared Beds24 -> Reservation pipeline (server-only).
//
// Extracted verbatim from app/api/bookings/route.ts so the stakeholder
// occupancy view (via buildReservationSet) uses the EXACT same grouping,
// room mapping and status logic as the dashboard - the two cannot drift.
...
export const APP_PHONE_MARKER = "[Created via Reporting App — Phone]";

export function mapChannel(apiSource = "", referer = "", comments = ""): Channel {
...
export function mergeGroupedBookings(all: Beds24Booking[]): Beds24Booking[] {
...
export function infoItemsText(items: unknown): string {
...
export function mapToReservation(b: Beds24Booking): Reservation {
  const nights =
...
// -- Composed reservation set for occupancy / cross-consumer consistency ------
// Same grouping + mapping + non-arrival overlay the dashboard GET applies,
...
export async function buildReservationSet(
  ...
  const raw = await fetchAllBookings(token, { fullSync: options.fullSync ?? false });
